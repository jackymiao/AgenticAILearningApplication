...
const ContentgraderSchema = z.object({ name: z.string(), user_name: z.string(), attempt_number: z.number(), score: z.number(), breakdown: z.array(z.object({ category_name: z.string(), deducted_points: z.number(), reason: z.string(), examples: z.array(z.string()) })), overview: z.string(), suggestions: z.array(z.string()) });
...
export const runWorkflow = async (workflow: WorkflowInput) => {
  return await withTrace("MultiAgentContent02", async () => {
    const state = {
      ...
      breakdown_rules: "Must always output exactly 5 breakdown objects. Use these exact category names every time, in this order:  Task Relevance  Main Ideas & Development  Support & Specificity  Accuracy & Consistency  Depth & Insight.  Categories must appear in the exact order listed above.  Each breakdown item must include:  category_name (must match exactly)  deducted_points (integer ≥ 0)  reason (short explanation; if 0 points, say “No significant issues found.”)  examples (0–3 short examples; empty array allowed)",
      overview_rules: "One paragraph that includes:  1–2 sentence summary of the essay’s message (neutral, no rewriting)  Top content issues (based on the deductions)  If previous essay exists: improvement/similar/declined + what changed in content quality  If no previous essay: explicitly say no comparison possible",
      suggestion_rules: "Provide 0–5 actionable suggestions  Prioritize the biggest deduction categories  Suggestions must be specific actions (e.g., “Add one concrete example of X”)  Do not rewrite the whole essay; do not add new facts not in essay",
      content_scoring_rules: "Start from 100 and deduct points based on content issues observed.  What “content” means here  Content evaluation includes:  Whether the essay answers the task (as far as can be inferred)  Strength and clarity of main message  Development of ideas (not just listing)  Use of specific support/examples  Logical consistency and factual/claim consistency inside the essay  Depth (goes beyond surface statements)  What NOT to grade here  Grammar correctness (unless it causes meaning loss)  Style/word choice elegance  Structure/organization (only mention if it harms content understanding, but keep it content-framed)  Deduction principles  Deduct once for recurring same issue pattern; note it as recurring.  Major content failure (off-topic, no main idea, contradictory claims) = bigger deduction.  Score must be integer 0–100.  Total deductions must equal 100 - score."
    };
    ...
    const runner = new Runner({
      traceMetadata: {
        ...
        workflow_id: "wf_696584d4bc988190abc1f4851da30b590dd2e99c2e45bc4a"
      }
    });
    ...
    const contentgraderResultTemp = await runner.run(
      ...
      [
        ...conversationHistory,
        {
          ...
          use ${state.content_scoring_rules} as guideline of scoring. 
  ...
  try {
    ...
    if (!result || !result.output_parsed) {
      return {
        ...
        error_message: 'Failed to get result from Content Agent workflow'
      };
    }
    ...
    return {
      status: 'error' as const,
      error_message: error instanceof Error ? error.message : 'Unknown error in Content Agent'
    };
  }
}
