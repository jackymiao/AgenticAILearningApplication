      ...
      try {
        ...
        if (detection.isAI) {
          ...
          res.status(400).json({
            ...
            details: 'This essay appears to be generated by AI. Please submit your own original work.',
            detection: {
              is_detected: detection.isAI,
              confidence: detection.confidence,
              confidence_category: detection.confidence > 0.7 ? 'HIGH' : detection.confidence > 0.4 ? 'MEDIUM' : 'LOW'
            }
          });
          return;
        }
