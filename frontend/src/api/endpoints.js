...
export const auth = {
  ...
  signup: (username, password, accessCode) =>
    apiFetch("/auth/admin/signup", {
      method: "POST",
...
export const publicApi = {
  ...
  validateStudent: (code, studentId, projectPassword) =>
    apiFetch(`/public/projects/${code}/validate-student`, {
      method: "POST",
      body: JSON.stringify({studentId, projectPassword}),
    }),
  getUserState: (code, userName) =>
    apiFetch(
      `/public/projects/${code}/user-state?userName=${encodeURIComponent(userName)}`,
    ),
  ...
  submitFinal: (code, userName, essay) =>
    apiFetch(`/public/projects/${code}/submissions/final`, {
      method: "POST",
...
export const gameApi = {
  initPlayer: (code, userName) =>
    apiFetch(`/game/projects/${code}/player/init`, {
      method: "POST",
      body: JSON.stringify({userName}),
    }),
  sendHeartbeat: (code, userName, sessionId) =>
    apiFetch(`/game/projects/${code}/heartbeat`, {
      method: "POST",
  ...
  defend: (code, attackId, useShield) =>
    apiFetch(`/game/projects/${code}/defend`, {
      method: "POST",
...
export const adminApi = {
  ...
  updateProjectStatus: (code, enabled) =>
    apiFetch(`/admin/projects/${code}/status`, {
      method: "PATCH",
  ...
  updateProject: (code, project) =>
    apiFetch(`/admin/projects/${code}`, {
      method: "PUT",
      body: JSON.stringify(project),
    }),
  importStudents: (code, file) => {
    ...
    return fetch(
      `${import.meta.env.VITE_API_BASE || "/api"}/admin/projects/${code}/students/import`,
      {
        method: "POST",
        body: formData,
        credentials: "include",
  ...
  deleteProjects: (codes) =>
    apiFetch("/admin/projects", {
      method: "DELETE",
      body: JSON.stringify({codes}),
    }),
  getSubmissions: (code, sort) =>
    apiFetch(`/admin/projects/${code}/submissions?sort=${sort || "newest"}`),
  getSubmission: (submissionId) =>
    apiFetch(`/admin/submissions/${submissionId}`),
  updateGrading: (submissionId, adminScore, adminFeedback) =>
    apiFetch(`/admin/submissions/${submissionId}/grading`, {
      method: "PATCH",
      body: JSON.stringify({adminScore, adminFeedback}),
    }),
  getFeedback: (code, sort) =>
    apiFetch(`/admin/projects/${code}/feedback?sort=${sort || "newest"}`),
};
